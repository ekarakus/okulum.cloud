...
@Component({
  selector: 'app-duty-location-add-edit-dialog',
  imports: [CommonModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatButtonModule, ReactiveFormsModule],
  template: `
    <h2 mat-dialog-title>{{ data.location ? 'Nöbet Yeri Düzenle' : 'Yeni Nöbet Yeri Ekle' }}</h2>
    <mat-dialog-content>
...
export class DutyLocationAddEditDialogComponent {
  form: FormGroup;

  constructor(
    private fb: FormBuilder,
    public dialogRef: MatDialogRef<DutyLocationAddEditDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: { location?: any }
  ) {
    this.form = this.fb.group({
      name: [data.location?.name || ''],
      description: [data.location?.description || ''],
      order: [data.location?.order ?? 0]
    });
  }
