        ...
        <div class="table-container">
          ...
          <thead>
            <tr style="background-color: #f5f5f5;">
              <th (click)="onHeaderClick('sort_order')" style="padding: 4px 12px; text-align: left; border-bottom: 1px solid #ddd; width:80px; cursor:pointer">Sıra No
                <mat-icon *ngIf="sort.field==='sort_order'" style="vertical-align: middle; font-size: 14px; margin-left:6px">{{ sort.dir==='asc' ? 'arrow_upward' : 'arrow_downward' }}</mat-icon>
              </th>
              <th (click)="onHeaderClick('name')" style="padding: 4px 12px; text-align: left; border-bottom: 1px solid #ddd; cursor:pointer">Özellik Adı
          ...
          <tbody>
            <tr *ngIf="pagedFeatures.length === 0">
              <td colspan="3" style="padding: 20px; text-align: center; color: #666;">
                Henüz özellik bulunmuyor.
              </td>
            </tr>
            <tr *ngFor="let feature of pagedFeatures" style="border-bottom: 1px solid #eee;">
              <td style="padding: 4px 12px; text-align:center"><strong>{{feature.sort_order}}</strong></td>
              <td style="padding: 4px 12px;"><strong>{{feature.name}}</strong></td>
              <td style="padding: 4px 12px;">{{feature.description}}</td>
              <td style="padding: 4px 12px;">
                <button mat-button color="accent" (click)="openEditDialog(feature)" style="margin-right: 5px;">
                  <span class="material-symbols-outlined" style="margin-right: 0.3rem; font-size: 18px;">edit</span>
                  Düzenle
                </button>
                <button mat-button color="warn" (click)="deleteFeature(feature.id)">
...
export class FeatureListComponent implements OnInit {
  ...
  private updatePagedData() {
    this.totalPages = Math.max(1, Math.ceil(this.features.length / this.pageSize));
    const start = this.pageIndex * this.pageSize;
    this.pagedFeatures = this.features.slice(start, start + this.pageSize);
  }
  ...
  private storageKey() {
    return isPlatformBrowser(this.platformId) ? `features_sort` : 'features_sort';
  }
  ...
  openAddDialog() {
    const dialogRef = this.dialog.open(FeatureAddEditDialogComponent, {
      width: '500px',
      data: { feature: null }
    });
  ...
  openEditDialog(feature: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.updateFeature(feature.id, result);
      }
    });
  }

  private addFeature(formData: any) {
    const token = this.getToken();
    if (token) {
      const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
      this.http.post(`${environment.apiUrl}/api/features`, formData, { headers }).subscribe({
        ...
        error: err => {
          ...
          this.snackBar.open('Özellik eklenirken bir hata oluştu', 'Kapat', { duration: 3500 });
        }
      });
    }
  }

  private updateFeature(id: number, formData: any) {
    const token = this.getToken();
    if (token) {
      const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
      this.http.put(`${environment.apiUrl}/api/features/${id}`, formData, { headers }).subscribe({
        ...
        error: err => {
          ...
          this.snackBar.open('Özellik güncellenirken bir hata oluştu', 'Kapat', { duration: 3500 });
        }
      });
    }
  }
      ...
      if (token) {
        ...
        this.http.delete(`${environment.apiUrl}/api/features/${id}`, { headers }).subscribe({
          ...
          error: err => {
            ...
            this.snackBar.open('Özellik silinirken bir hata oluştu', 'Kapat', { duration: 3500 });
          }
        });
      }
    }
